...
app.use(express.json({ limit: "32kb" })); app.use(healthRouter); app.use("/api/payments", paymentsRouter); app.use(webhooksRouter); app.use(reconciliationRouter); app.use(supportRouter); app.use((_req, res) => res.status(404).json({ error: "Not found" }));
