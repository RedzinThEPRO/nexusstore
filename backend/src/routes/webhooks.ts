...
router.post("/webhooks/evopay", async (req, res) => {
  const parsed = evoPayWebhookSchema.safeParse(req.body);
  if (!parsed.success) return res.status(400).json({ error: "Invalid EvoPay webhook payload" });
  ...
  if (eventType === "AMOUNT_MISMATCH") { console.error("EvoPay reconciliation error: amount mismatch", { transactionId: event.id, orderId: payment.order_id, expectedAmount: payment.amount, receivedAmount: event.amount }); return res.status(200).json({ received: true, matched: true, processed: false }); }
  if (payment.status === "PAID") return res.status(200).json({ received: true, duplicate: true, alreadyPaid: true });

  if (event.status === "COMPLETED") {
    let confirmed;
    try { confirmed = await evoPay.getPixTransaction(event.id); }
    catch (error) { if (error instanceof EvoPayError) console.error("EvoPay confirmation unavailable", { transactionId: event.id, status: error.status }); else console.error("EvoPay confirmation failed", { transactionId: event.id }); return res.status(202).json({ received: true, matched: true, processed: false }); }
    ...
    const { error: paidError } = await db.from("payments").update({ status: "PAID", provider_status: "COMPLETED", paid_at: new Date().toISOString() }).eq("id", payment.id).eq("status", "PENDING");
    if (paidError) { console.error("EvoPay payment update failed", { transactionId: event.id, orderId: payment.order_id }); return res.status(202).json({ received: true, matched: true, processed: false }); }
    const { error: orderError } = await db.from("orders").update({ payment_status: "PAID", status: "PAID" }).eq("id", payment.order_id).eq("payment_status", "PENDING");
    if (orderError) { console.error("EvoPay order update failed", { transactionId: event.id, orderId: payment.order_id }); return res.status(202).json({ received: true, matched: true, processed: false }); }
  } else {
    const mapped = mapPaymentStatus(event.status);
    const { error } = await db.from("payments").update({ status: mapped, provider_status: event.status }).eq("id", payment.id).eq("status", "PENDING");
