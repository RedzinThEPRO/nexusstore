...
const transactionSchema = z.object({
  ...
  amountWithTax: z.number().finite().positive().optional(),
...
export class EvoPayService {
  ...
  async createPixCharge(input: EvoPayChargeInput): Promise<EvoPayTransaction> {
    const response = await fetch(BASE_URL + "/pix", { method: "POST", headers: { "API-Key": this.apiKey, "Content-Type": "application/json" }, body: JSON.stringify(input) });
    return this.read(response);
  }
  async getPixTransaction(id: string): Promise<EvoPayTransaction> {
    const response = await fetch(BASE_URL + "/pix?id=" + encodeURIComponent(id), { headers: { "API-Key": this.apiKey, "Content-Type": "application/json" } });
    return this.read(response);
  }
  private async read(response: Response): Promise<EvoPayTransaction> {
    ...
    if (!parsed.success) throw new EvoPayError(502, "Invalid response from EvoPay");
    return parsed.data;
  }
}
